		...
		if (bean != null) {
			if (!this.EmailExists(bean.getEmail())) {
				...
				EmailUtil.sendEmail(bean.getEmail(),"StarBusterCinima 會員帳號電子信箱驗證信件",
						"<p>此為系統自動發送的信件，並不接收回信，請勿回覆。</p><br /><p>Hi, "+bean.getName()+"</p><p>請點擊下方連結完成帳號的啟用</p> <a href='http://localhost:8080/StarBusterCinima/member/EmailVerify.controller/?userEmail="+userEmail+"&emailEncryption="+emailEncryption+"'>請點選此處進行驗證</a><br /><p>注意事項：</p><p>1. 若您沒有申請這項服務，請忽略這封信件。</p><p> 2. 如收到多封信件，請以最新的一封進行操作。</p><p>3. 請於兩天內點選以下連結完成帳號啟用，超過期限該連結將自動失效。</p><p> 4. 若連結無法直接點擊，請手動複製連結貼至瀏覽器網址前往。</p>", null);
	...
	public MemberBean updateMemberInfo(MemberBean bean) {// 更改會員個人資料
		if (bean != null) {
			return memberDAO.update(bean.getMemberId(), bean.getEmail(), bean.getPassword(), bean.getName(),
					...
					bean.getRegistTime(), bean.getFilmAdType(), bean.getGiftAdType(),bean.getIdentityStatus(),bean.getIdentityCode());
		}
		return bean;
	}
}
